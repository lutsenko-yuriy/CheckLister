...
- [wip] N/A-70 (WU1): Theme foundation — light/dark `Palette`, `ThemeProvider`/`useTheme()` following the OS appearance, `createThemedStyles()`, and navigation chrome/status bar now follow the OS setting. Individual screens are migrated in later WUs.
